...
export async function POST(request: Request) {
  ...
  if (!payload.id || !Number.isInteger(payload.version) || !["new", "working", "done"].includes(payload.status ?? "") || !["do", "change", "no", "ack"].includes(payload.action ?? "")) return Response.json({ error: "Invalid action" }, { status: 400 });
  ...
  const status = payload.action === "no" ? "rejected" : payload.action === "ack" ? "acknowledged" : "working";
  ...
  if (payload.action === "no" || payload.action === "ack") {
